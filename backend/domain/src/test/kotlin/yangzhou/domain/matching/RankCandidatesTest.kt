...
class RankCandidatesTest {
    ...
    private val jcw30 = Item(
        id = "JCW-30",
        title = "auth baseline",
        requirements = listOf(Requirement("Java", 3), Requirement("架构", 2)),
    )

    @Test
    fun `fewer missing first then smaller total delta (S6)`() {
        val 小李 = Member("小李", listOf(Capability("Java", 3), Capability("架构", 2)))   // green, 0/0
        val 小张 = Member("小张", listOf(Capability("Java", 2), Capability("架构", 3)))   // yellow, 0/1
        val me = Member("我", listOf(Capability("Java", 4), Capability("架构", 1)))      // yellow, 0/1
    ...
    fun `candidate with a gap outranks one missing the skill entirely (S7)`() {
        val me = Member("我", listOf(Capability("Java", 4)))                              // missing UI 设计
        val 小王 = Member("小王", listOf(Capability("UI 设计", 2)))                        // gap 1
        val booUi = Item("BOO-UI", "UI 设计", listOf(Requirement("UI 设计", 3)))
